...
# With variable cropping ie. we will get a frame fixed in size but objects zoom in and zoom out when rolling is done
...
# Fusion parameters
ALPHA = 0.98  # Complementary filter balance (accel vs gyro)
prev_time = tt()
filtered_roll = 0.0
...
def mpu_init():
    bus.write_byte_data(MPU_ADDR, PWR_MGMT_1, 0)  # Wake up MPU6050
...
# ðŸŸ¢ ADDED: Helper function to compute safe cropping dimensions
...
def generate_frames():
    ...
    while True:
        ...
        # ðŸŸ¢ MODIFIED: Rotate frame using stabilized angle
        (h, w) = frame.shape[:2]
        M = cv2.getRotationMatrix2D((w / 2, h / 2), display_ang, 1.0)
        rotated = cv2.warpAffine(frame, M, (w, h), flags=cv2.INTER_LINEAR)

        # ðŸŸ¢ ADDED: Compute dynamic cropping size based on roll offset
        angle_offset = abs((roll - 180 + 360) % 360 - 180)  # 0 at rest, max 90Â°
        crop_w, crop_h = compute_safe_crop(w, h, angle_offset)

        # ðŸŸ¢ ADDED: Crop centered region from rotated frame
        x1 = (w - crop_w) // 2
        y1 = (h - crop_h) // 2
        cropped = rotated[y1:y1 + crop_h, x1:x1 + crop_w]

        # ðŸŸ¢ ADDED: Optional resize to maintain output dimension
        output = cv2.resize(cropped, (w, h), interpolation=cv2.INTER_LINEAR)

        # ðŸŸ¢ MODIFIED: Overlay crop info for debug
        status = f"Roll: {roll:.1f}Â°, Crop: {crop_w}x{crop_h}"
...
# ðŸŸ¢ MODIFIED: Start sensor thread
